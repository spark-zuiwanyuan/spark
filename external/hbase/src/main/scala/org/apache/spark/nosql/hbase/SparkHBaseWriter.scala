	...
	if (record.length == columns.length + 1) {
	  ...
	  List.range(1, record.length) foreach {
	    i => put.add(columns(i - 1).family, columns(i - 1).qualifier,
	    		toByteArray(record, i, columns(i - 1).typ))
	  }
	  htable.put(put)
	} else {
 ...
 * @param typ type [[org.apache.spark.sql.catalyst.types.DataType]]
 */
class HBaseColumn(val family: Array[Byte], val qualifier: Array[Byte], val typ: DataType)
